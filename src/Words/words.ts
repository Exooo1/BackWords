...
const getWords = async (req: ReqQueryType<{count:string}> & userIDType,
...
const addWord = async (req: ReqBodyType<{ word: string }> & userIDType, res: Response<StatusType<string>>) =>{
    try {
        const {word} = req.body
...
const deleteWord = async(req: ReqQueryType<{ id: string; letter: string }> & userIDType,
...
const wordChange = async(req: ReqBodyType<{ word: string; translate: string; description: string; id: String }> &
                        userIDType,
                    res: Response<StatusType<string>>) =>{
    try {
...
const wordFind = async (req: ReqQueryType<{ word: string }> & userIDType, res:Response<StatusType<Array<WordType>>>)=>{
    try {
        const profile = (await authModel.findOne({_id: req.userId})) as AccountType
        if (!profile) return res.status(404).json(status<null>(null, 0, 'NotFound'))
...
const sortWords = async( req: ReqBodyType<{ isSort: boolean; sortType: 'ADDED' | 'DESCRIPTION' }> & userIDType,
                    res:Response<StatusType<Array<WordType>>>) =>{
    try {
        const profile = (await authModel.findOne({_id: req.userId})) as AccountType
        if (!profile) return res.status(404).json(status<null>(null, 0, 'NotFound'))
        const array: Array<WordType> = []
...
const wordsDownload = async(req: Request & userIDType, res:Response<StatusType<string>>)=>{
    try {
        const profile = (await authModel.findOne({_id: req.userId})) as AccountType
        if (!profile) return res.status(404).json(status<null>(null, 0, 'NotFound'))
        const array: Array<WordType> = []
        ...
        if (result.length >= 1) res.status(200).send(status(result,1,''))
    } catch (err) {
        res.status(500).json(status<null>(null, 0, err))
    }
}

export {getWords,addWord,deleteWord,wordChange,wordFind,sortWords,wordsDownload}
